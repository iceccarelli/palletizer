#!/usr/bin/env bash
# =============================================================================
#  Robot & Cell Showcase (FULL LIBRARY) — one-shot installer.
#  Uses EVERY unique image across all zips: dedupes, optimizes to WebP
#  (full + thumbnail), generates a typed manifest, writes the tabbed
#  Featured / All cells / Feature story component, builds, and pushes.
#  Safe: stashes your work, builds on a clean branch off origin/main.
# =============================================================================
set -euo pipefail

[ -d web ] && [ -d .git ] || { echo "ERROR: run from the repo root (must contain web/ and .git/)." >&2; exit 1; }
BRANCH="feat/robot-cell-showcase"
SRC=/tmp/showcase_src

# ---- 1. rescue uncommitted work -------------------------------------------
if [ -n "$(git status --porcelain)" ]; then
  echo ">> Stashing your uncommitted work"
  git stash push -u -m "wip-before-showcase-$(date +%Y%m%d-%H%M%S)" || true
fi

# ---- 2. clean branch off latest origin/main -------------------------------
echo ">> Creating a clean '$BRANCH' from origin/main"
git fetch origin --quiet
git checkout -B "$BRANCH" origin/main

# ---- 3. extract EVERY zip at repo root ------------------------------------
echo ">> Extracting all image zips"
rm -rf "$SRC"; mkdir -p "$SRC"
shopt -s nullglob
for z in *.zip; do
  d="$SRC/$(basename "$z" .zip)"
  mkdir -p "$d"
  unzip -o -q "$z" -d "$d" || true
done
shopt -u nullglob
echo "   extracted $(find "$SRC" -type f \( -iname '*.png' -o -iname '*.jpg' -o -iname '*.jpeg' \) | wc -l | tr -d ' ') image files"

# ---- 4. optimize + build manifest (Python + Pillow) -----------------------
echo ">> Installing Pillow (image optimizer)"
...
echo ">> Optimizing images to WebP + writing manifest"
SHOWCASE_SRC="$SRC" python3 <<'PY'
import glob, hashlib, os, json
from PIL import Image
SRC=os.environ["SHOWCASE_SRC"]
OUT="web/public/showcase"
for d in ("featured","gallery","story"):
    os.makedirs(f"{OUT}/{d}", exist_ok=True)
os.makedirs("web/lib", exist_ok=True)

def save2(path, base):
    im=Image.open(path).convert("RGB")
    full=im.copy()
    if full.width>1100: full=full.resize((1100,int(full.height*1100/full.width)))
    full.save(base+".webp","WEBP",quality=82,method=4)
    th=im.copy(); w=520
    if th.width>w: th=th.resize((w,int(th.height*w/th.width)))
    th.save(base+"@t.webp","WEBP",quality=72,method=4)

def imgs(d):
    return sorted(glob.glob(f"{d}/**/*.png",recursive=True)
                 +glob.glob(f"{d}/**/*.jpg",recursive=True)
                 +glob.glob(f"{d}/**/*.jpeg",recursive=True))

# featured = the base independent_10 set
feat_dir=os.path.join(SRC,"palletizer_independent_10")
feat=imgs(feat_dir)[:10]
for i,f in enumerate(feat,1): save2(f,f"{OUT}/featured/{i:02d}")

# all unique images across every zip
seen=set(); uniq=[]
for f in imgs(SRC):
    h=hashlib.sha256(open(f,"rb").read()).hexdigest()
    if h in seen: continue
    seen.add(h); uniq.append(f)
landing=[f for f in uniq if "landing-images" in f]
robots=[f for f in uniq if f not in landing]

labels=["Full-cell overview","Infeed pick","Mixed-SKU build","Guarded line-end","Dual-station changeover",
        "Lights-out shift","Layer close-up","Twin-pallet cell","High-density top-off","Sanitary cell"]
gallery=[]
for i,f in enumerate(robots,1):
    save2(f,f"{OUT}/gallery/g{i:03d}")
    gallery.append({"src":f"/showcase/gallery/g{i:03d}.webp","thumb":f"/showcase/gallery/g{i:03d}@t.webp","label":labels[i%len(labels)]})

story_meta={
 "01":("End-of-Line Cell","A complete palletizing cell — one software layer driving arm, gripper, vision and safety."),
 "02":("Precision AI Gripper","Vision-guided tooling that seals, lifts and seats each case within the confidence gate."),
 "03":("AI Mixed-SKU Pallet","Live optimization packs mixed cases into a dense, physics-validated stack."),
 "04":("One Codebase, Any Robot","The same engine drives any supported arm through a sub-100-line driver."),
 "05":("Global Fleet Command","Every cell, every site — monitored and orchestrated from one control plane."),
 "06":("Safety-First Collaboration","Guarding, light curtains and a 2.5 s watchdog keep people and product safe."),
 "07":("High Throughput","Sustained ~120 cases/hr per cell without sacrificing stack stability."),
 "08":("RobotInterface < 100 Lines","Onboard a new arm by implementing one tiny, well-documented interface."),
 "09":("Food & Beverage Sanitary","Washdown-rated tooling and per-case traceability for hygiene-critical lines."),
 "10":("E-commerce Mixed-SKU","Extreme SKU variety, peak-season ready, maximum cases per pallet."),
 "11":("Digital Twin, Sim-to-Real","Validate patterns in simulation, then run the identical plan on hardware."),
 "12":("Scale to Global Fleets","From a single pilot cell to a worldwide, audited deployment."),
}
story=[]
for f in sorted(landing):
    key=os.path.basename(f)[:2]
    t,d=story_meta.get(key,("Palletizer","Autonomous palletizing."))
    save2(f,f"{OUT}/story/{key}")
    story.append({"src":f"/showcase/story/{key}.webp","thumb":f"/showcase/story/{key}@t.webp","title":t,"desc":d})

ts ="// AUTO-GENERATED by setup-showcase. Do not edit by hand.\n"
ts+="export type GalleryItem = { src: string; thumb: string; label: string };\n"
ts+="export type StoryItem = { src: string; thumb: string; title: string; desc: string };\n\n"
ts+="export const GALLERY: GalleryItem[] = "+json.dumps(gallery,indent=2)+";\n\n"
ts+="export const FEATURE_STORY: StoryItem[] = "+json.dumps(story,indent=2)+";\n"
open("web/lib/showcaseManifest.ts","w").write(ts)
print(f"   featured 10 | gallery {len(gallery)} | story {len(story)}")
PY
...
# ---- 6. integrate into page.tsx + Navbar.tsx (idempotent) -----------------
echo ">> Integrating into app/page.tsx and components/Navbar.tsx"
node <<'NODE_EOF'
const fs=require('fs');
const pP='web/app/page.tsx'; let p=fs.readFileSync(pP,'utf8');
if(!p.includes('RobotShowcase')){
  p=p.replace('import { SectionHeader, SectionShell, Eyebrow, SectionTitle, Card } from "@/components/Section";',
              'import { SectionHeader, SectionShell, Eyebrow, SectionTitle, Card } from "@/components/Section";\nimport RobotShowcase from "@/components/RobotShowcase";');
  p=p.replace('      {/* SOLUTIONS / INDUSTRIES with Unsplash style backgrounds */}',
  ...
  fs.writeFileSync(pP,p); console.log('   page.tsx: integrated');
} else console.log('   page.tsx: already integrated');
const nP='web/components/Navbar.tsx'; let n=fs.readFileSync(nP,'utf8');
if(!n.includes('/#showcase')){
  n=n.replace('const PLAIN_LINKS = [\n  { href: "/#solutions", label: "Solutions" },',
              'const PLAIN_LINKS = [\n  { href: "/#showcase", label: "Showcase" },\n  { href: "/#solutions", label: "Solutions" },');
  fs.writeFileSync(nP,n); console.log('   Navbar.tsx: integrated');
} else console.log('   Navbar.tsx: already integrated');
NODE_EOF
...
git add web/components/RobotShowcase.tsx web/lib/showcaseManifest.ts web/app/page.tsx web/components/Navbar.tsx web/public/showcase
git commit -m "feat(web): full-library Robot & Cell Showcase — 94 optimized images, tabbed carousel + gallery + lightbox"
git push -u origin "$BRANCH" --force-with-lease

echo ""
echo "============================================================"
echo " DONE — open the PR link above. Featured(10) + All cells + Feature story."
echo " Your WIP is safe in the stash; fix/consistency-foundation untouched."
echo "============================================================"
